---
name: exp-status
description: "Read-only status rollup for MetriFi GEO experiments and client deliverables: for one experiment or a whole team, report where each stands, what the last operator noted, which of the four pre-publish checks are recorded, missing, or stale, and above all what is waiting on a human. Use whenever someone asks to check in or pick work back up: 'what needs me', 'what is waiting on me', 'where are we with this team', 'status of the HELOC experiment', 'check in on an experiment', 'what is the latest', 'did the client respond', 'resume where we left off', 'what is in flight', 'give me a rollup'. Answers from platform state only, never from local files, so any operator gets the same answer from any machine. It never creates, edits, sends, re-runs prompts, or records a check; when it finds obvious next work it names the skill that does it and stops. NOT for running a phase, NOT for site or page status (Site Builder skills), and NOT a campaign analytics report."
---
