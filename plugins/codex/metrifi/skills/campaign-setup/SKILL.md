---
name: campaign-setup
description: "Stand up a MetriFi GEO team's campaigns and the prompts inside them, before any experiment exists. Owns new-team onboarding: pick the most populated market the institution serves, confirm the client's own organization is registered so its responses get scored, and build the FLAGSHIP campaign, wide across the products that matter in one market. Every later campaign goes narrow, one product each, and experiments attach those granular prompts, never the flagship's. Every prompt names its geography in its own text; the campaign location only buys keyword demand and never reaches the LLM providers. Use when a team is new or a campaign is being created or added to: 'set up this new team', 'onboard this client in GEO', 'create a campaign', 'add prompts to this campaign', 'get a baseline for this institution', 'which market should we track', 'why is everything reading zero'. NOT for scoring an opportunity (exp-build), NOT for demand triage on a topic campaign (exp-research), NOT website work."
---
