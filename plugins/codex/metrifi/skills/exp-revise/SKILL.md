---
name: exp-revise
description: "Pick up what a client did to a MetriFi GEO deliverable and turn it into a revision: read the deliverables with unprocessed client activity, apply the client's answers as methodology inputs rather than literal edit commands, re-verify anything they merely confirmed, push the revised article, and re-run every pre-publish check the edit staled. Use when a client has responded or a live deliverable needs another round: 'the client answered', 'apply the client answers', 'revise the deliverable', 'push a revision', 'they left comments', 'the compliance officer replied', 'they confirmed the checklist', 'they approved it', 'they asked to opt out', 'run the revise loop', 'check for client responses'. Safe to run on a cadence and after a crash. A client confirming something is never a verification, an answer that fails the methodology is returned with an explanation, and an opt-out halts publication. NOT the first send (exp-deliver), NOT the client's sign-off, NOT for website pages."
---
