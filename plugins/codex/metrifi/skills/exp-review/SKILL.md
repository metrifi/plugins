---
name: exp-review
description: "Run the four pre-publish checks on a MetriFi GEO deliverable's draft article and record each verdict, which is what lets the deliverable be sent: hygiene (AI tells, spelling, markdown), NCUA and lending compliance, accessibility (WCAG 2.1 AA), and fact verification against the institution's live site. Use when a draft is ready for its pre-publish pass, or when someone doubts one: 'review the draft', 'run the review battery', 'is this ready to ship', 'is this compliant', 'compliance check', 'NCUA review', 'fact-check the article', 'accessibility check', 'ADA check', 'proofread this', 'check it for AI tells', 'why is the send warning about checks', 'the checks went stale', 're-run the checks'. Writes each report as an experiment document and records each verdict. Assistive, never authoritative: a compliance officer at the institution signs off, and it proposes fixes rather than rewriting. NOT the send step, NOT for reviewing a page design (Site Builder skills), NOT a substitute for the institution's own review."
---
