---
name: exp-sweep
description: "The daily cross-client sweep over every MetriFi GEO top customer: read the whole cohort in one call, decide which teams have work in motion and which have gone quiet, then move every team as far down the experiment pipeline as it can go without a person, stopping only at the gates that genuinely belong to a human. Hands each team to the phase skill that owns the work (exp-revise when a client responded, exp-build when baseline responses landed, exp-review when checks are missing or stale, exp-research when a team has nothing running) and never re-implements a phase itself. Starts a new experiment on its own for a team that has gone quiet, choosing the topic from measured visibility gaps and handing over a complete brief so the research phase never stalls asking a question nobody is awake to answer. Use whenever someone wants the whole book of business moved or reviewed at once: 'run the daily sweep', 'sweep all clients', 'what is waiting on me across every client', 'move everything forward', 'which clients have nothing running', 'kick off experiments for anyone who has gone quiet', 'daily rollup', 'what is ready to send', 'check every team'. Safe to run on a schedule and safe to re-run: it re-derives the board from platform state every time, holds no memory of its own, and a sweep with nothing to do says so in three lines. It never sends anything to a client, never approves a plan or a draft, and never crosses a human gate; the pile it hands you at the end is deliverables ready for your send decision. NOT a per-experiment status check (that is exp-status), NOT a phase (those are exp-research, exp-build, exp-review, exp-revise, exp-deliver), and NOT a campaign analytics report."
---
