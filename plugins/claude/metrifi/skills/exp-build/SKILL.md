---
name: exp-build
description: "Phase two of a MetriFi GEO experiment: take a campaign whose baseline responses are in and turn it into a defensible client deliverable. Scores every prompt on measured demand, on whether models name any specific financial institution, and on the gap in the client's own content, then opens with a Viability Verdict (STRONG, VIABLE, WEAK, or AVOID) and locks the biggest viable target. On WEAK or AVOID it pivots rather than shipping a weak target. Then it writes the opportunity block and the evidence and decision documents, drafts the article against the institution's verified live site, and assembles the client page. Use once responses have populated: 'analyze the baseline', 'the responses are in', 'score the opportunity', 'which prompt should we target', 'write the article', 'draft the deliverable', 'build the client page', 'lock the target', 'pivot this experiment'. It never sends to a client. NOT for creating a campaign (exp-research), NOT the pre-publish checks (exp-review), NOT sending (exp-deliver)."
---
