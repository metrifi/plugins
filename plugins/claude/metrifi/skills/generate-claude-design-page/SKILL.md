...
- **One design by default; more when asked.** Converge on a single resolved design unless the requester asks for options ("three variations", "a couple of directions") — then build that many. Every variation sits on the same approved design system and meets the same rubric; they differ in layout and emphasis, never in brand. They are alternatives to pick between: exactly one becomes the site's `prototype-page`. Don't redirect this ask elsewhere (`designer-loop` is for a human tasting variants interactively, which is a different thing).
