...
- **exp-review keeps the fix out of a finding's `issue`.** `issue` states the defect and `suggested_fix` states the remedy. The client page prints resolved findings back verbatim under "found and fixed", so an `issue` carrying its own remedy showed a Honda FCU reader "Fix: remove the term figure; keep terms and rates on the linked rates page", an internal instruction wearing the client's finding. Nothing truncates the text, because cutting a sentence would misrepresent the record, so the rule belongs where the finding is written.
